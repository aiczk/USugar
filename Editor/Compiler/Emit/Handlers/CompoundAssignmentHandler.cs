    ...
    CLeaf VisitCompoundAssignment(ICompoundAssignmentOperation op)
    {
            ...
            && cuOpM.ContainingType is INamedTypeSymbol cuOpCt && _lowering.IsObjectArrayEmulated(cuOpCt))
        {
            var res = _lowering.EmitCallToMethod(
                ...
                new List<CLeaf> { leftVal, rightVal });
            lv.Write(res);
            return res;
        }
    ...
    CLeaf VisitEventAssignment(IEventAssignmentOperation op)
    {
            ...
            && _lowering.Planner.InterfaceIsLocalUserClassOnly(localInterface))
        {
            ...
            if (targets.Count == 1)
            {
                _lowering.EmitExprStmt(_lowering.EmitCallToMethod(
                    _lowering.RequireRegisteredCallable(targets[0].Impl),
            ...
            foreach (var target in targets)
            {
                ...
                _lowering.Builder.EmitIf(match, _ => _lowering.EmitExprStmt(_lowering.EmitCallToMethod(
                    _lowering.RequireRegisteredCallable(target.Impl),
            ...
            || evt.RemoveMethod == null || !evt.RemoveMethod.IsImplicitlyDeclared)
        {
            ...
            if (objectArrayOwner)
            {
                ...
                {
                    _lowering.EmitExprStmt(_lowering.EmitCallToMethod(
                        _lowering.RequireRegisteredCallable(accessor),
