...
public sealed class ResolvedEdgeResolver
{
        ...
        switch (site.Kind)
        {
            ...
            default:
                throw new System.ArgumentOutOfRangeException(
                    nameof(site), site.Kind,
                    "No target resolution for this callable-site kind; an unresolved site drops "
                    + "its call edge from reach, recursion and spill analysis.");
        }
    }
    ...
    IEnumerable<IMethodSymbol> AccessorDispatchImplDefs(IPropertyReferenceOperation pr, IMethodSymbol acc)
    {
        ...
        if (overYield)
        {
            ...
            foreach (var vt in _emitter.VirtualDispatchInstance.Resolve(
                CallableSites.Require(pr, acc), arecv).RuntimeTargets)
                yield return vt.Impl.OriginalDefinition;
    }
    ...
    IEnumerable<IMethodSymbol> ClassToStringImplDefs(IOperation valueOp)
    {
        ...
        if (ClassTypeObjectContext.ContainsTypeParameter(t))
        {
            ...
            foreach (var vt in _emitter.VirtualDispatchInstance.Resolve(
                CallableSites.Synthetic(CallableSiteKind.Method, slotDef, valueOp),
                nts).RuntimeTargets)
    ...
    // Definition-keyed override targets for the reachability graph. BoundProgram later stores the
    // corresponding constructed target for emission.
    ...
    IPropertySymbol LeafPropertyTarget(IPropertyReferenceOperation pr)
    {
        ...
        var cand = VirtualDispatch.FindOverridePropertyInChain(
            _emitter.ClassSymbol, def, p.Name);
        if (cand == null) return null;
        return SymbolEqualityComparer.Default.Equals(cand.OriginalDefinition, def) ? null : cand.OriginalDefinition;
    }

    // The graph is definition-keyed; constructed targets are materialized by call-site binding.
    IMethodSymbol ResolveLeafOverrideDef(IMethodSymbol def)
        => VirtualDispatch.FindOverrideMethodInChain(
               _emitter.ClassSymbol, def, def.Name)
               ?.OriginalDefinition
           ?? def;
