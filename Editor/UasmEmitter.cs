...
public class UasmEmitter
{
    ...
    // Property shims → EmitContext (facade still uses these for EmitFields/EmitMethods/EmitMethod)
    Compilation _compilation => _ctx.Compilation;
    INamedTypeSymbol _classSymbol => _ctx.ClassSymbol;
    UasmModule _module => _ctx.Module;
    VariableTable _vars => _ctx.Vars;
    LayoutPlanner _planner => _ctx.Planner;
    Dictionary<IMethodSymbol, int> _methodLabels => _ctx.MethodLabels;
    ...
    List<(IMethodSymbol symbol, int label)> _pendingLocalFunctions => _ctx.PendingLocalFunctions;
    ...
    Dictionary<IMethodSymbol, int> _methodBodyLabels => _ctx.MethodBodyLabels;
    HashSet<IMethodSymbol> _inheritedMethods = new(SymbolEqualityComparer.Default);
    List<(string fieldId, IOperation initOp, ITypeSymbol fieldType)> _fieldInitOps => _ctx.FieldInitOps;
    Dictionary<string, string> _fieldChangeCallbacks => _ctx.FieldChangeCallbacks;
    Dictionary<ILabelSymbol, int> _gotoLabels => _ctx.GotoLabels;
    ...
    // Safe to call when _typeParamMap is null (falls through to non-map overload).
    ...
    public string Emit()
    {
        _module.SetHeader($"Generated by USugar");
        EnsurePlannerReady();
        EmitFields();
        SetReflectionValues();
        EmitMethods();
    ...
    void SetReflectionValues()
    {
        ...
        _vars.SetReflectionValues(typeId, typeName);

        var ancestorIds = CollectAncestorTypeIds(_classSymbol);
        if (ancestorIds.Length > 1)
            _vars.DeclareReflTypeIds(ancestorIds);
    }
    ...
    public uint GetHeapSize() => _module.GetHeapSize();

    public VariableTable Variables => _vars;
    ...
    void EmitFields()
    {
        foreach (var member in _classSymbol.GetMembers().OfType<IFieldSymbol>())
        {
            ...
            _vars.DeclareField(member.Name, udonType, flags, constDefault, syncMode);
            if (heapValue != null)
                _vars.SetConstValue(member.Name, heapValue);
                ...
                && fcbAttr.ConstructorArguments[0].Value is string propName)
            {
                ...
                _vars.DeclareField($"__old_{member.Name}", udonType);
            }
        }
        ...
        foreach (var prop in _classSymbol.GetMembers().OfType<IPropertySymbol>())
        {
            ...
            _vars.DeclareField(prop.Name, udonType, flags);
        }
        ...
        while (baseType != null)
        {
            ...
            foreach (var member in baseType.GetMembers().OfType<IFieldSymbol>())
            {
                ...
                _vars.DeclareField(member.Name, udonType, baseFlags, constDefault);
    ...
    void EmitMethods()
    {
        ...
        foreach (var method in methods)
        {
            ...
            var label = _module.DefineLabel(exportName);
            ...
            bool hasVarChangeCallback = method.MethodKind == MethodKind.PropertySet
                ...
                && _fieldChangeCallbacks.ContainsValue(setProp.Name);
            ...
            if (willExport)
                _methodBodyLabels[method] = _module.DefineLabel(ml.BodyLabel);
            ...
            for (int i = 0; i < method.Parameters.Length; i++)
            {
                ...
                _vars.DeclareVar(ml.ParamIds[i], udonType);
            ...
            if (!method.ReturnsVoid && ml.ReturnId != null)
            {
                ...
                _vars.DeclareVar(ml.ReturnId, retType);
        ...
        _vars.DeclareConst("SystemUInt32", "0xFFFFFFFF");
        ...
        if ((_fieldInitOps.Count > 0 || _fieldChangeCallbacks.Count > 0) && !methods.Any(m => UdonEventNames.TryGetValue(m.Name, out var en) && en == "_start"))
        {
            var label = _module.DefineLabel("_start");
            _module.AddExport("_start", label);
            _module.MarkLabel(label);
            var sentinel = _vars.DeclareConst("SystemUInt32", "0xFFFFFFFF");
            _module.AddPush(sentinel);
            ...
            var startBodyLabel = _module.DefineLabel("_start__body");
            _module.AddLabel(startBodyLabel);
            _module.MarkLabel(startBodyLabel);
        ...
        foreach (var (ifaceMethod, ifaceMl, classMl) in bridges)
        {
            ...
            for (int i = 0; i < ifaceMethod.Parameters.Length; i++)
            {
                if (ifaceMl.ParamIds[i] != classMl.ParamIds[i])
                {
                    ...
                    _vars.TryDeclareVar(ifaceMl.ParamIds[i], udonType);
                }
            }
            if (ifaceMl.ReturnId != null && ifaceMl.ReturnId != classMl.ReturnId)
            {
                var retType = GetUdonType(ifaceMethod.ReturnType);
                _vars.TryDeclareVar(ifaceMl.ReturnId, retType);
            }

            // Export the interface name → bridge trampoline
            var bridgeLabel = _module.DefineLabel(ifaceMl.ExportName);
            _module.AddExport(ifaceMl.ExportName, bridgeLabel);
            _module.MarkLabel(bridgeLabel);
            ...
            {
                if (ifaceMl.ParamIds[i] != classMl.ParamIds[i])
                    _module.AddCopy(ifaceMl.ParamIds[i], classMl.ParamIds[i]);
            }

            // Call class body via returnJump so control returns here after body executes
            if (_methodBodyLabels.TryGetValue(
                    _classSymbol.FindImplementationForInterfaceMember(ifaceMethod) as IMethodSymbol,
                    out var bodyLabel))
            {
                ...
                if (needsReturnCopy)
                {
                    ...
                    _module.AddPush(sentinel);
                    var bridgeContinuation = _module.DefineLabel($"__bridge_continue_{ifaceMl.ExportName}");
                    _module.AddPushLabel(bridgeContinuation);
                    _module.AddJump(bodyLabel);
                    _module.MarkLabel(bridgeContinuation);

                    // Copy class return → interface return
                    _module.AddCopy(classMl.ReturnId, ifaceMl.ReturnId);
    ...
    void EmitMethod(IMethodSymbol method)
    {
        ...
        var label = _methodLabels[method];
        var exportName = _methodVarPrefix[method]; // stored from first pass (matches label name)
        ...
        if (shouldExport)
        {
            ...
            if (fcbFieldName != null)
            {
                ...
                var varChangeLabel = _module.DefineLabel(varChangeExportName);
                _module.AddExport(varChangeExportName, varChangeLabel);
                _module.MarkLabel(varChangeLabel);

                // Preamble: swap new/old values
                // 1. field → value param (new value to setter parameter)
                var paramId = _methodParamVarIds[method][0];
                _module.AddCopy(fcbFieldName, paramId);
                // 2. _old_ → field (restore field to old value)
                _module.AddCopy($"__old_{fcbFieldName}", fcbFieldName);
                // Fall through to setter body
            }

            _module.AddExport(exportName, label);
            _module.MarkLabel(label);
            ...
            var bodyLabel = _methodBodyLabels[method];
            _module.AddLabel(bodyLabel);
            _module.MarkLabel(bodyLabel);
        }
        else if (_methodBodyLabels.TryGetValue(method, out var bodyLbl))
        {
            // Explicit interface impl: only mark body label (bridge handles export)
            _module.AddLabel(bodyLbl);
            _module.MarkLabel(bodyLbl);
        }
        else
        {
        ...
        if (syntaxRef != null)
        {
            ...
            else if (bodyOp is IAnonymousFunctionOperation anonFunc)
            {
                ...
                else if (anonFunc.Body != null && _methodRetVars.TryGetValue(method, out var lambdaRetId))
                {
                    ...
                    _module.AddCopy(resultId, lambdaRetId);
                }
            }
                     ...
                     && propDecl.ExpressionBody != null)
            {
                ...
                if (exprOp != null && _methodRetVars.TryGetValue(method, out var retId))
                {
                    ...
                    _module.AddCopy(resultId, retId);
                }
            }
        ...
        if (fcbFieldName != null)
            _module.AddCopy(fcbFieldName, $"__old_{fcbFieldName}");
    ...
    void EmitFieldInitializers()
    {
        foreach (var (fieldId, initOp, fieldType) in _fieldInitOps)
        {
            try
            {
                ...
                if (initOp is IArrayInitializerOperation arrayInit)
                {
                    ...
                    var sizeConst = _vars.DeclareConst("SystemInt32", arrayInit.ElementValues.Length.ToString());
                    _module.AddPush(sizeConst);
                    _module.AddPush(fieldId);
        ...
        foreach (var kvp in _fieldChangeCallbacks)
            _module.AddCopy(kvp.Key, $"__old_{kvp.Key}");
    }
    ...
    {
        foreach (var entry in _vars.GetAllEntries())
            _module.DeclareVariable(entry.Id, entry.UdonType, entry.DefaultValue, entry.Flags, entry.SyncMode, entry.ConstValue);
    }

    void SyncOptimizerConsts()
    {
        var constValues = _module.GetConstValues();
        var varTypes = _module.GetVariableTypes();
        foreach (var kv in constValues)
        {
            if (!varTypes.TryGetValue(kv.Key, out var type)) continue;
            _vars.TryDeclareVar(kv.Key, type);
            _vars.SetConstValue(kv.Key, kv.Value);
        }
    }
}
