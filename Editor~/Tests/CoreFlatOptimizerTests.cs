...
/// <summary>
/// Flat (post-flatten) Core IR optimizer tests: CoalesceSlots over FlatBlocks (the only retained flat
...
public class CoreFlatOptimizerTests
{
    ...
    static FlatModule MakeModule(FlatFunction func)
    {
    ...
    static FlatFunction MakeFunc(string name = "test") => new(name);
    ...
    public void Coalesce_NonOverlapping_Merged()
    {
        ...
        bb0.Instructions.Add(new CAssign(0, new CConst(10, StorageTypes.Int32)));            // pos 0: def slot0
        bb0.Instructions.Add(new CStoreField("f1", new CSlotRef(0, StorageTypes.Int32)));    // pos 1: use slot0 (last use)
        bb0.Instructions.Add(new CAssign(1, new CConst(20, StorageTypes.Int32)));            // pos 2: def slot1
        bb0.Instructions.Add(new CStoreField("f2", new CSlotRef(1, StorageTypes.Int32)));    // pos 3: use slot1 (last use)
        ...
        var move2 = Assert.IsType<CAssign>(bb0.Instructions[2]);
        Assert.Equal(0, move2.DestSlot);

        // And the fourth instruction reads slot0
        var store2 = Assert.IsType<CStoreField>(bb0.Instructions[3]);
    ...
    public void Coalesce_RepresentationCopyReadPreservesInterference()
    {
        ...
        bb0.Instructions.Add(new CRepresentationCopy(
            2,
            new CSlotRef(0, StorageTypes.Int32),
            StorageTypes.String,
            RepresentationCastKind.ClosedGenericObjectCast));
        bb0.Instructions.Add(new CStoreField("number", new CSlotRef(1, StorageTypes.Int32)));
        bb0.Terminator = new CRet();

        CoreFlatOptimizer.CoalesceSlots(MakeModule(func));

        Assert.NotEqual(
            ((CAssign)bb0.Instructions[0]).DestSlot,
            ((CAssign)bb0.Instructions[1]).DestSlot);
    }
    ...
    public void Coalesce_RepresentationCopyRemapsSourceSlot()
    {
        ...
        var bb0 = func.NewBlock();
        bb0.Instructions.Add(new CAssign(0, new CConst(10, StorageTypes.Int32)));
        bb0.Instructions.Add(new CStoreField("first", new CSlotRef(0, StorageTypes.Int32)));
        bb0.Instructions.Add(new CAssign(1, new CConst(20, StorageTypes.Int32)));
        bb0.Instructions.Add(new CRepresentationCopy(
            2,
        ...
        var copy = Assert.IsType<CRepresentationCopy>(bb0.Instructions[3]);
    ...
    public void Coalesce_Overlapping_Kept()
    {
        ...
        var bb0 = func.NewBlock();
        bb0.Instructions.Add(new CAssign(0, new CConst(10, StorageTypes.Int32)));            // pos 0: def slot0
        bb0.Instructions.Add(new CAssign(1, new CConst(20, StorageTypes.Int32)));            // pos 1: def slot1 (slot0 still live)
        ...
        var es = Assert.IsType<CExprStmt>(bb0.Instructions[2]);
    ...
    public void Coalesce_DifferentTypes_NotMerged()
    {
        ...
        var bb0 = func.NewBlock();
        bb0.Instructions.Add(new CAssign(0, new CConst(42, StorageTypes.Int32)));
        bb0.Instructions.Add(new CStoreField("f1", new CSlotRef(0, StorageTypes.Int32)));
        bb0.Instructions.Add(new CAssign(1, new CConst(true, StorageTypes.Boolean)));
        bb0.Instructions.Add(new CStoreField("f2", new CSlotRef(1, StorageTypes.Boolean)));
        bb0.Terminator = new CRet();
    ...
    public void Coalesce_Pinned_NeverMerged()
    {
        ...
        bb0.Instructions.Add(new CStoreField("f1", new CSlotRef(0, StorageTypes.Int32)));
        bb0.Instructions.Add(new CAssign(1, new CConst(20, StorageTypes.Int32)));
        bb0.Instructions.Add(new CStoreField("f2", new CSlotRef(1, StorageTypes.Int32)));
        bb0.Terminator = new CRet();
        ...
        // Operands unchanged
        var move1 = Assert.IsType<CAssign>(bb0.Instructions[0]);
    ...
    public void Coalesce_LoopHeaderRedefinedSlot_MergesAcrossDeadBody()
    {
        ...
        header.Instructions.Add(new CLoadField(0, "cond", StorageTypes.Int32));
        ...
        body.Instructions.Add(new CAssign(1, new CConst(42, StorageTypes.Int32)));
        body.Instructions.Add(new CStoreField("result", new CSlotRef(1, StorageTypes.Int32)));
        ...
        var bodyMove = Assert.IsType<CAssign>(body.Instructions[0]);
    ...
    public void Coalesce_LoopCarriedSlot_LiveAcrossBody_DoesNotMerge()
    {
        ...
        pre.Instructions.Add(new CLoadField(0, "base", StorageTypes.Int32));
        pre.Terminator = new CJump(header.Id);

        // header: slot2 = cond; branch
        header.Instructions.Add(new CLoadField(2, "cond", StorageTypes.Boolean));
        ...
        body.Instructions.Add(new CAssign(1, new CConst(42, StorageTypes.Int32)));
        body.Instructions.Add(new CStoreField("a", new CSlotRef(1, StorageTypes.Int32)));
        body.Instructions.Add(new CStoreField("b", new CSlotRef(0, StorageTypes.Int32)));
        ...
        var bodyMove = Assert.IsType<CAssign>(body.Instructions[0]);      // slot1 def
        var carriedRead = Assert.IsType<CStoreField>(body.Instructions[2]); // slot0 read
    ...
    public void Coalesce_LoopWithNonOverlapping_StillMerges()
    {
        ...
        header.Instructions.Add(new CLoadField(2, "flag", StorageTypes.Boolean));
        header.Terminator = new CBranch(new CSlotRef(2, StorageTypes.Boolean), body.Id, exit.Id);

        // body: use slot0 entirely within body
        body.Instructions.Add(new CAssign(0, new CConst(10, StorageTypes.Int32)));
        body.Instructions.Add(new CStoreField("x", new CSlotRef(0, StorageTypes.Int32)));
        body.Terminator = new CJump(header.Id);

        // exit: use slot1 entirely after loop
        exit.Instructions.Add(new CAssign(1, new CConst(20, StorageTypes.Int32)));
        exit.Instructions.Add(new CStoreField("y", new CSlotRef(1, StorageTypes.Int32)));
        exit.Terminator = new CRet();
        ...
        var bodyMove = Assert.IsType<CAssign>(body.Instructions[0]);
        var exitMove = Assert.IsType<CAssign>(exit.Instructions[0]);
    ...
    public void Coalesce_RewritesOperands()
    {
        ...
        bb0.Instructions.Add(new CAssign(0, new CConst(10, StorageTypes.Int32)));            // def slot0
        bb0.Instructions.Add(new CStoreField("f1", new CSlotRef(0, StorageTypes.Int32)));    // last use slot0

        // slot1: def after slot0 is dead → should coalesce to slot0
        bb0.Instructions.Add(new CAssign(1, new CConst(20, StorageTypes.Int32)));            // def slot1
        bb0.Instructions.Add(Call(null,
        ...
        bb0.Instructions.Add(new CAssign(2, new CConst(true, StorageTypes.Boolean)));
        ...
        var es = Assert.IsType<CExprStmt>(bb0.Instructions[3]);
        var call = Assert.IsType<CExternCall>(es.Expr);
    ...
    public void Coalesce_NestedLoop_DualLevelCarriedAccumulators_NoInterferenceAndMergeCorrect()
    {
        ...
        bb0.Instructions.Add(new CAssign(0, new CConst(0, StorageTypes.Int32)));
        bb0.Instructions.Add(new CAssign(1, new CConst(0, StorageTypes.Int32)));
        bb0.Terminator = new CJump(bb1.Id);

        bb1.Instructions.Add(Call(5, "SystemInt32.__op_LessThan__SystemInt32_SystemInt32__SystemBoolean",
        ...
        bb2.Instructions.Add(new CAssign(2, new CConst(0, StorageTypes.Int32)));
        bb2.Instructions.Add(new CAssign(3, new CConst(0, StorageTypes.Int32)));
        bb2.Terminator = new CJump(bb3.Id);

        bb3.Instructions.Add(Call(6, "SystemInt32.__op_LessThan__SystemInt32_SystemInt32__SystemBoolean",
        ...
        bb4.Instructions.Add(Call(4, "SystemInt32.__op_Addition__SystemInt32_SystemInt32__SystemInt32",
            new List<CLeaf> { new CSlotRef(1, StorageTypes.Int32), new CSlotRef(3, StorageTypes.Int32) }, "SystemInt32"));
        bb4.Instructions.Add(Call(7, "SystemInt32.__op_Addition__SystemInt32_SystemInt32__SystemInt32",
            new List<CLeaf> { new CSlotRef(2, StorageTypes.Int32), new CSlotRef(4, StorageTypes.Int32) }, "SystemInt32"));
        bb4.Instructions.Add(new CAssign(2, new CSlotRef(7, StorageTypes.Int32)));
        bb4.Instructions.Add(Call(8, "SystemInt32.__op_Addition__SystemInt32_SystemInt32__SystemInt32",
            new List<CLeaf> { new CSlotRef(3, StorageTypes.Int32), new CConst(1, StorageTypes.Int32) }, "SystemInt32"));
        bb4.Instructions.Add(new CAssign(3, new CSlotRef(8, StorageTypes.Int32)));
        bb4.Terminator = new CJump(bb3.Id); // inner back-edge

        bb5.Instructions.Add(Call(9, "SystemInt32.__op_Addition__SystemInt32_SystemInt32__SystemInt32",
            new List<CLeaf> { new CSlotRef(0, StorageTypes.Int32), new CSlotRef(2, StorageTypes.Int32) }, "SystemInt32"));
        bb5.Instructions.Add(new CAssign(0, new CSlotRef(9, StorageTypes.Int32)));
        bb5.Instructions.Add(Call(10, "SystemInt32.__op_Addition__SystemInt32_SystemInt32__SystemInt32",
            new List<CLeaf> { new CSlotRef(1, StorageTypes.Int32), new CConst(1, StorageTypes.Int32) }, "SystemInt32"));
        bb5.Instructions.Add(new CAssign(1, new CSlotRef(10, StorageTypes.Int32)));
        ...
        var outerAccDest = ((CAssign)bb0.Instructions[0]).DestSlot;
        var iDest = ((CAssign)bb0.Instructions[1]).DestSlot;
        var innerAccDest = ((CAssign)bb2.Instructions[0]).DestSlot;
        var jDest = ((CAssign)bb2.Instructions[1]).DestSlot;
    ...
    public void Coalesce_DoWhile_BackEdgeKeepsCarriedValueDistinctFromBodyTemp()
    {
        ...
        entry.Instructions.Add(new CAssign(0, new CConst(0, StorageTypes.Int32)));
        entry.Terminator = new CJump(body.Id);

        body.Instructions.Add(new CAssign(1, new CConst(1, StorageTypes.Int32)));
        body.Instructions.Add(Call(3, "SystemInt32.__op_Addition__SystemInt32_SystemInt32__SystemInt32",
            new List<CLeaf> { new CSlotRef(0, StorageTypes.Int32), new CSlotRef(1, StorageTypes.Int32) },
            "SystemInt32"));
        body.Instructions.Add(new CAssign(0, new CSlotRef(3, StorageTypes.Int32)));
        body.Instructions.Add(Call(2, "SystemInt32.__op_LessThan__SystemInt32_SystemInt32__SystemBoolean",
        ...
        var accumulator = ((CAssign)entry.Instructions[0]).DestSlot;
        var temp = ((CAssign)body.Instructions[0]).DestSlot;
    ...
    public void Coalesce_UsedBeforeDef_NotMergedWithEarlierNaivelyNonOverlappingSlot()
    {
        ...
        bb0.Instructions.Add(new CAssign(2, new CConst(true, StorageTypes.Boolean)));
        bb0.Instructions.Add(new CAssign(1, new CConst(5, StorageTypes.Int32)));
        bb0.Instructions.Add(new CStoreField("f1", new CSlotRef(1, StorageTypes.Int32))); // slot1 dead right here
        bb0.Terminator = new CJump(bb1.Id);

        bb1.Terminator = new CBranch(new CSlotRef(2, StorageTypes.Boolean), bb2.Id, bb3.Id);

        bb2.Instructions.Add(new CAssign(0, new CConst(42, StorageTypes.Int32)));
        bb2.Terminator = new CJump(bb4.Id);

        bb3.Terminator = new CJump(bb4.Id); // never assigns slot0

        bb4.Instructions.Add(new CStoreField("result", new CSlotRef(0, StorageTypes.Int32)));
        ...
        var slot0Dest = ((CAssign)bb2.Instructions[0]).DestSlot;
        var slot1Dest = ((CAssign)bb0.Instructions[1]).DestSlot;
