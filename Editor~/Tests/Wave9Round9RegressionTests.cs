...
/// [Y6]  using/Dispose inside a base generic open-T body ICEd ("No CFunction registered for
...
public class W9R9InhRec : W9R9InhRecBase {
    ...
    {
        // MinUsingInhGenOpenT (ICE "No CFunction registered for method 'Dispose'" pre-fix).
        var uasm = TestHelper.CompileToUasm(@"
using System;
using UdonSharp;
...
public class W9R9TLf : W9R9TLfBase {
    ...
    [Fact]
    public void CapturingLambda_OwnClassVariableReceiver_Rejects()
    {
        // MinE1_VarRecvCaptureStore (compiled clean pre-fix — W9P25 regression flavor).
        var ex = Record.Exception(() => TestHelper.CompileToUasm(@"
using System;
using UdonSharp;
public class W9R9VarRecv : UdonSharpBehaviour {
    ...
    void Start() {
        ...
        Assert.NotNull(ex);
        Assert.Contains(CaptureStoreFragment, ex.ToString());
    }

    [Fact]
    public void CapturingLambda_BaseTypedReceiver_Rejects()
    {
        // MinE2_BaseRecvCaptureStore (W9P25 twin).
        var ex = Record.Exception(() => TestHelper.CompileToUasm(@"
using System;
using UdonSharp;
public class W9R9BaseRecvBase : UdonSharpBehaviour { public Func<int, int> hook; }
public class W9R9BaseRecv : W9R9BaseRecvBase {
    public int seed; public int result;
    void Start() {
        int k = seed + 1;
        W9R9BaseRecvBase b = this;
        b.hook = x => x + k;
        result = hook(seed);
    }
}", "W9R9BaseRecv"));
    ...
    [Fact]
    public void CapturingLocalFunctionGroup_VariableReceiver_Rejects()
    {
        // MinE5_VarRecvCaplfStore: the method-group flavor of the same escape channel.
        var ex = Record.Exception(() => TestHelper.CompileToUasm(@"
using System;
using UdonSharp;
public class W9R9LfRecv : UdonSharpBehaviour {
    public int seed; public int result;
    Func<int, int> hook;
    void Start() {
        int k = seed + 1;
        int Lf(int x) { return x + k; }
        W9R9LfRecv v = this;
        v.hook = Lf;
        result = hook(seed);
    }
    ...
    [Fact]
    public void CapturingLambda_ForeignFieldReceiver_Rejects()
    {
        // W10CF9ProbeSimpleForeign (P1): behaviour FIELD receiver, simple-assign arm.
        var ex = Record.Exception(() => TestHelper.CompileToUasm(@"
using System;
using UdonSharp;
public class W9R9Foreign : UdonSharpBehaviour {
    W9R9Foreign other;
    public Func<int, int> cb;
    public int n; public int total;
    void Start() {
        other = this;
        int lv = n + 1;
        other.cb = x => x + lv;
    ...
    [Fact]
    public void CapturingLambda_ForeignFieldTupleDecon_Rejects()
    {
        // W10CF9ProbeDirectTupleForeign (P3): the tuple-decon leg of the same channel.
        var ex = Record.Exception(() => TestHelper.CompileToUasm(@"
using System;
using UdonSharp;
public class W9R9ForeignTup : UdonSharpBehaviour {
    W9R9ForeignTup other;
    public Func<int, int> cb;
